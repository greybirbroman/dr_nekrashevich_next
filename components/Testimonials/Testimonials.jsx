...
useEffect(() => {
  ...
  return (
    <section id='testimonials' className='p-4 md:p-8 lg:p-12 cursor-default flex flex-col items-center bg-slate-50'>
      ...
      <ul className='flex sm:flex-col flex-wrap justify-center gap-10'>
        {testimonials.slice(0, visibleCount).map((item, index) => (
          ...
          >
              <blockquote className='p-4 sm:text-[16px] text-[20px] font-normal text-gray-700 leading-8'>&#8220;{item.text}&#8221;</blockquote>
              <div className='flex justify-between items-center bg-cyan-700 text-white h-[100px] p-4'>
                ...
                <Link href='https://yandex.ru/profile/149051823874' target='_blank'>
                <Image src='https://yastatic.net/s3/home-static/_/Z/6/nfVezDRqofwQZ5e669DSK84Tw.svg' alt='Яндекс, логотип' width={60} height={60} className='max-w-[100%] h-auto'/>
                </Link>
                ...
                <div className='flex flex-col items-center'>
                  ...
                  <div className='flex'>
                  {stars.map((_, index) => (
                      <Image key={index} src='/star.svg' alt='Изображение, звезда, рейтинг' width={20} height={20}/>
                    ))}

                  </div>
                </div>
                <Image src='/quote.svg' alt='Изображение, ковычки, цитата' width={60} height={60} />
              </div>
          </m.li>
        ))}
      </ul>
      {isMobileResolution &&
      <PrimaryButton title={isNotShowAll ? 'Показать все' : 'Cкрыть'} onClick={handleShowAll}/>}
    </section>
  );
};
